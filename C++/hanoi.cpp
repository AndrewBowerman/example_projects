...
bool checkEven(int n) {	//This function checks if a given value is even, that is if the
...
void moveOne(char source, char destination) {	//This function will move a single disk from the
...
void solveTower(int n, int moves, char source, char spare, char destination) {
	bool z = checkEven(n);	
	if (!z) {	//If the number of disks is not even, this portion will run
		for (int i1 = 1; i1 <=moves;i1++) {	//Run a for loop between 1 and total number of moves
...
solveTower(n,totalmoves,'A','B','C');	//Uses the last peg, peg C as the destination peg and this
